...
export default function WizardLivePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(wizardDataUrl('agents.json')).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('scenario.json')).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('build_script.json')).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }),
    ]).then(([a, s, b]) => {
      setAgents(a.agents);
      setScenario(s);
      setEvents(b.events);
  ...
  return (
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        <section
          className="research-card flex flex-col no-lift lg:!h-[calc(100dvh-440px)]"
